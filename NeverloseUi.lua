...
if CoreGui:FindFirstChild("AAS1q_sADD") then
	GlobalWindow = CoreGui:FindFirstChild("AAS1q_sADD")
else
	GlobalWindow = Instance.new('ScreenGui');
end

-- ProtectGui(GlobalWindow);

GlobalWindow.Name = "AAS1q_sADD";
...
GlobalWindow:SetAttribute("data", "u")
...
NeverLose.Flags = {};
NeverLose.AccentColor = Color3.fromRGB(255, 147, 35);
NeverLose.MainColor = Color3.fromRGB(13, 11, 8);
...
NeverLose.SetIconMode = LPH_NO_VIRTUALIZE(function(self , Label: TextLabel , Icon: string)
    if type(Icon) == "string" and Icon:find("rbxassetid://") then
        local img = Label:FindFirstChild("IconImage")
        if not img then
        ...
        img.Image = Icon
        img.ImageColor3 = Label.TextColor3
        img.Visible = true
        Label.Text = ""
        return
    end
...
function NeverLose:CreateOptionWindow(Frame: Frame , Zindex)
	...
	OptionHandler.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
...
function NeverLose:CreateColorPicker(HandleFrame: Frame)
	...
	ColorPickerHandler.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
	...
	RGBLabel.BackgroundColor3 = Color3.fromRGB(36, 32, 26)
...
function NeverLose:RegisiterHandler(Handler: Frame , Signal)
	...
	function handle:AddToggle(Config)
		...
		Toggle.BackgroundColor3 = Color3.fromRGB(21, 17, 10)
		...
		ToggleLib.SetUI = LPH_NO_VIRTUALIZE(function(value)
			if value then
				...
				NeverLose.PlayAnimate(Toggle,SlowyTween,{
					BackgroundTransparency = 0,
					BackgroundColor3 = Color3.fromRGB(21, 17, 10)
				})
	...
	function handle:AddButton(Config)
		...
		ButtonFrame.BackgroundColor3 = Color3.fromRGB(33, 28, 25)
		...
		NeverLose:CreateInput(ButtonFrame, function()
			Config.Callback();
		end)
		...
		Button.SetRender(Signal:GetValue())
	...
	function handle:AddSlider(Config)
		...
		Slider.BackgroundColor3 = Color3.fromRGB(36, 32, 26)
		...
		ValueFrame.BackgroundColor3 = Color3.fromRGB(36, 32, 26)
		...
		SlideFrame.BackgroundColor3 = Color3.fromRGB(36, 32, 29)
	...
	function handle:AddKeybind(Config)
		...
		Keybind.BackgroundColor3 = Color3.fromRGB(36, 32, 26)
	...
	function handle:AddTextInput(Config)
		...
		TextInput.BackgroundColor3 = Color3.fromRGB(36, 32, 26)
	...
	function handle:AddDropdown(Config)
		...
		Dropdown.BackgroundColor3 = Color3.fromRGB(36, 32, 26)
		...
		do
			...
			DropdownHandler.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
...
function NeverLose:CreateToolTips(Container: Frame , Name: string , Content: string)
	...
	Tooltips.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
...
function NeverLose:RegisiterItem(Frame: Frame , Signel)
	...
	function idx:AddLabel(Name: string,Warp: boolean)
		...
		BasedFrame.BackgroundColor3 = Color3.fromRGB(33, 28, 25)
	...
	function idx:AddUserFrame(Name : string , Profile: string , Expires : string)
		...
		UserFrame.BackgroundColor3 = Color3.fromRGB(33, 28, 25)
...
function NeverLose:CreateWindow(Config)
	...
	WindowFrame.BackgroundColor3 = Color3.fromRGB(13, 11, 8)
	...
	RightMenuFrame.BackgroundColor3 = Color3.fromRGB(13, 11, 8)
	...
	ConfigFrame.BackgroundColor3 = Color3.fromRGB(22, 18, 13)
	...
	function Window:AddTab(Config)
		...
		TabButton.BackgroundColor3 = Color3.fromRGB(49, 44, 41)
		...
		function Tab:AddSection(Config)
			...
			SectionHandler.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
	...
	function Window:_InitConfig()
		...
		ConfigMenu.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
		...
		InputFrame.BackgroundColor3 = Color3.fromRGB(33, 28, 25)
		...
		OpenButton.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
	...
	function Window:Watermark()
		...
		Watermark.BackgroundColor3 = Color3.fromRGB(13, 11, 8)
...
function NeverLose:CreateNotification()
	...
	function Notifier.new(Config)
		...
		NotifyFrame.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
...
function NeverLose:CreateLogger()
	...
	function Logging.new(IconStr: string , Message: string , Duration: number)
		...
		LogFrame.BackgroundColor3 = Color3.fromRGB(27, 24, 20)
...
function NeverLose:CreateIndicator()
	...
	function Indicators.new(Config)
		...
		IndicatorItem.BackgroundColor3 = Color3.fromRGB(13, 11, 8)
...
-- }).Body)()
Library = a()
local SaveFolder = "Sapphire/" .. tos(GameId)
...
function Neverlose:CreateLogger(_Text)
	return Library:CreateLogger(_Text);
end
...
function Neverlose:LoadUi(_Title: string, _Desc: string, _Icon: string, _Size: any, _Keybind: string)
	local createSapphireToggle = function()
		...
		ImageButton.Position = UDim2.new(0.5, 0, 0.081349209, 0)
		ImageButton.Size = UDim2.new(0, 60, 0, 60)
		ImageButton.Image = "rbxassetid://75211319139068"
	...
	if not CoreGui:FindFirstChild("Sapp") then
		createSapphireToggle()
	end

	if game:GetService("CoreGui").RobloxGui:FindFirstChild("AAS1q_sADD") then
...
function Neverlose:CreateButton(where, data: table)
...
function Neverlose:SetAccount(_Window: any, data: table)
	return _Window:SetAccount({Profile = data.Profile, Username = data.Username, Expires = data.Expires})
end
...
function Neverlose:CreateToggle(where, data)
	...
	if Config[data.Title] == nil then
		Config[data.Title] = data.Default or false
	end
	local function Handle(state)
		...
		if fn then
			if state then
				if not Threads[data.Title] then
					Threads[data.Title] = task.spawn(fn)
				end
			else
				if Threads[data.Title] then
					task.cancel(Threads[data.Title])
					Threads[data.Title] = nil
				end
			end
		end
...
function Neverlose:CreateSlider(where, data)
	...
	if data.Callback then
		data.Callback(Config[data.Title])
	end
	return slider
end
...
function Neverlose:CreateDropdown(where, data)
	...
	if data.Callback then
		if data.Multi then
			local selected = {}
			for key, value in pairs(Config[data.Title]) do
				if value then
					table.insert(selected, key)
				end
			end
...
Config = Neverlose:GetConfig()
AllFuncs = Neverlose:GetAllFuncs()

AllFuncs["Auto Farm"] = function()
    while Config["Auto Farm"] do task.wait()
        print("A")
    end
end

Neverlose:SetNewData(Config, AllFuncs)
Neverlose:LoadConfig()

local Window = Neverlose:LoadUi("Sapphire.cc", "discord.gg/mpz8atSMKt", "rbxassetid://75211319139068")
Neverlose:CreateTabLabel(Window, "Test")

local Logging = Neverlose:CreateLogger()

local Watermark = Neverlose:CreateWatermark(Window)
local Ping = Watermark:AddBlock("chart-four-vertical-bars" , "0MS");
local UITogg = Watermark:AddBlock("cube-vertexes" , "Ui Toggle");
...
task.spawn(function()
    ...
    while true do task.wait(1)
        ...
        frames = 0
        last = now
        Ping:SetText("FPS: "..math.floor(fps))
    end
end)
...
local Tabs = {
    MainTab = Neverlose:CreateTab(Window, "Main", "crosshairs"),
    MainTab2 = Neverlose:CreateTab(Window, "Main2", "crosshairs")
}

local FarmSection = Neverlose:CreateSection(Tabs.MainTab, "Farming", "left")

Neverlose:CreateToggle(FarmSection, {
    Title = "Auto Farm",
    Callback = function(v)end,
})

Neverlose:CreateButton(FarmSection, {
	Title = "Test",
	Icon = "crosshairs",
	Callback = function(v)
		print("A")
...
Neverlose:CreateSlider(FarmSection, {
    Title = "Distance",
    Nums = {[100] = "Max"},
    Min = 0,
    Max = 100,
...
local SelectMobDropdown = Neverlose:CreateDropdown(FarmSection, {
    Title = "Select Mob",
    Values = {"Mob1", "Mob2", "Mob3"},
    Multi = false,
    Callback = function(v)
...
SelectMobDropdown:SetValues({"A"})

Neverlose:CreateNotification({
    Title = "Key Verify",
    Duration = 5,
})
...
Window.UserSettings:AddLabel('Menu Scale'):AddDropdown({
	...
	Callback = function(v)
		Window:SetSize(Neverlose:GetUiScales()[v]);
...
Window.UserSettings:AddButton({
	...
	Name = 'Discord',
	Callback = function()
		setclipboard("discord.gg/mpz8atSMKt")
...
task.spawn(function()
    while true do task.wait(1/60)
        local seconds = LRM_SecondsLeft or 0
        local expireText
        ...
        elseif seconds == math.huge then
            ...
            expireText = days .. "Day " .. hours .. "h " .. minutes .. "m"
        end
